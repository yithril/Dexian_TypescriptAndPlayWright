...
function shippingCost(book: Book): number {
  switch (book.format) {
    case 'physical':
...
function deliver(book: Book): string {
  switch (book.format) {
    case 'physical':
...
function returnPolicy(book: Book): string {
  switch (book.format) {
    case 'physical':
...
if (toggleBtn) {
  toggleBtn.addEventListener('click', (e) => {
    ...
    (e.target as HTMLButtonElement).textContent = open ? 'Hide the explainer' : 'Show the explainer';
  });
}
